...
class MCTS_Tree_Node():
    ...
    def legal(self, act):
        ...
        xy = [new_loc.x, new_loc.y, 0]

        mxy = self.obstacle_map.world2vox(np.array(xy))
        local_obs_map = self.obstacle_map._tsdf_vol_cpu

        # Check that new location is within the map bounds
        if mxy[0] < 0 or mxy[0] >= local_obs_map.shape[0] or\
            mxy[1] < 0 or mxy[1] >= local_obs_map.shape[1]:
            return False
